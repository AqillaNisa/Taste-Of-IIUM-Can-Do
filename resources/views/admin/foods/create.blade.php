                ...
                <form action="{{ route('admin.foods.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="stall_id" name="stall_id">
                                ...
                                @foreach ($stalls as $stall)
                                    <option value="{{ $stall->id }}" @selected((string) old('stall_id') === (string) $stall->id)>{{ $stall->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mt-1">
                        <div class="col-md-6">
                            ...
                            <input class="form-control" id="category" name="category" value="{{ old('category') }}" placeholder="Western, Dessert, Beverages..." required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="price">Price</label>
                            <input class="form-control" id="price" name="price" type="number" min="0" step="0.01" value="{{ old('price', '0.00') }}" required>
                        </div>
                    </div>
